...
public interface ReplyBoardRepository extends JpaRepository<ReplyBoard, Integer> {
    // 계층형 조회 쿼리 @Query(, nativeQuery=true)
    ...
    // TODO : 게시물 저장 함수 : 최초 생성 (board_group(그룹번호), board_parent(부모번호))
    //                  => board_group(부모번호 == 자식번호(bid)), board_parent(0(최초생성), 부모번호)
    //      JPA insert 문 직접 작성(dml : 테이블 데이터 변경, 트랜잭션 동반)
    //      ==> @Transactional, @Modifying
    @Transactional
    @Modifying
    @Query(value = "INSERT INTO TB_REPLY_BOARD " +
            "VALUES(sq_reply_board.nextval, :#{#replyBoard.boardTitle}," +
            ":#{#replyBoard.boardContent}, :#{#replyBoard.boardWriter}," +
            "0, sq_reply_board.CURRVAL, " +
            "0, 'N', TO_CHAR(SYSDATE, 'YYYY-MM-DD HH24:MI:SS')," +
            " NULL, NULL)", nativeQuery = true)
    int insertByBoard(@Param("replyBoard") ReplyBoard replyBoard);

    // TODO : 게시물 + 답변글 2개 삭제 만들기 : update 문 직접 작성
    @Transactional
    @Modifying
